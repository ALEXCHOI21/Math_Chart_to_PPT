...
STEP 1. 좌표계를 먼저 정확히 확정하십시오 (가장 중요: 이미지 전체 경계 기준)
- image_x_range: 원본 이미지 프레임의 가장 왼쪽 끝 가장자리부터 오른쪽 끝 가장자리까지의 실제 수학적 x 좌표 범위 [x_min, x_max]
- image_y_range: 원본 이미지 프레임의 가장 아래쪽 끝 가장자리부터 위쪽 끝 가장자리까지의 실제 수학적 y 좌표 범위 [y_min, y_max]
- 이 값들은 단순 축(x축, y축)이 그려진 길이가 아닙니다! 이미지 테두리 전체의 수학적 범위를 유추해야 합니다.
- (예: 원점 O가 이미지 가로 30%, 세로 30% 위치에 있다면, image_x_range는 [-3, 7], image_y_range는 [-3, 7]처럼 원점 기준 여백을 포함해야 합니다.)
- 이 경계 좌표가 정확해야만 생성된 결과물이 원본과 픽셀 단위로 정확히 겹쳐집니다.
...
[
  {
    "chart_id": 1,
    "axes": {
      "image_x_range": [xmin, xmax],
      "image_y_range": [ymin, ymax],
      "origin_label": "O",
      "x_label": "x",
      "y_label": "y"
    },
...
5. axes의 image_x_range, image_y_range가 이미지 전체 프레임 테두리의 수학적 좌표와 일치하는가?
